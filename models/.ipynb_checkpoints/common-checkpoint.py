...
from reslcnet import ResLCnet50
...
class Convffn(nn.Module):
    def __init__(self, c1, c2, k=1, s=1):
        ...
        # self.dwconv = nn.Conv2d(c1, c2, 1, 1)
        # self.bn = nn.BatchNorm2d(c1)
        self.conv3 = nn.Conv2d(c1, c2, kernel_size=k,stride=s, bias=False)
        self.gelu = nn.ReLU()
        self.conv4 = nn.Conv2d(c2, c2, kernel_size=k,stride=s, bias=False)
    def forward(self, x):
...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv3 = Convffn(2 * c_, c2, 1)  # optional act=FReLU(c2)
...
class reslcnet(nn.Module):
    def __init__(self,slice):
        ...
        self.model = None
        if slice == 1:
            self.model = ResLCnet50().features[:11]
        elif slice == 2:
            self.model = ResLCnet50().features[11:12]
        else:
            self.model = ResLCnet50().features[12:]
    def forward(self,x):
        return self.model(x)
